...
def banner():
	...
	print(design.renderText('PE-Tracker'))
	print('┌──────────────────────────────────────┐')
	print('│++++ You are your own rule maker. ++++│')
	print('└──────────────────────────────────────┘')
...
def display(track_dict, threshold_data):
	...
	print('\n┌' + dash + '┐')
	print('{:^30}'.format(threshold_list[0]))
	print('├' + dash + '┤')
	...
	if not os.path.exists(PATH + 'template.json'):
		...
		if(option.lower() == 'y'):			
			...
			if(today_stat.lower() == 'y'):
				...
				if(NOD == 0):
					print('Congratulations, You\'ve completed your resolution successfully')
					...
					if(previous_record[list(previous_record)[0]]['Date'] != DATE.strftime("%a, %d-%b-%Y")):
						...
						print('You have set the stats already for the day.')
